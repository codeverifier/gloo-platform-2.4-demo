...
install_gloo_platform() {
    ...
    if [[ "$should_support_vault" == true ]]; then
        ...
        value=\$value NLB_LB_SCHEME_TYPE=$MGMT_NLB_LB_SCHEME_TYPE NLB_LB_ADDRESS_TYPE=$MGMT_NLB_LB_ADDRESS_TYPE envsubst < <(cat $DIR/core/gloo-platform/vault/gloo-platform-mgmt-plane-2.4.yaml) | helm upgrade --install gloo-platform gloo-platform/gloo-platform \
            --kube-context=${MGMT_CONTEXT} \
            --namespace=gloo-mesh \
            --create-namespace \
            --version=${GLOO_PLATFORM_HELM_VERSION} \
        ...
        value=\$value NLB_LB_SCHEME_TYPE=$MGMT_NLB_LB_SCHEME_TYPE NLB_LB_ADDRESS_TYPE=$MGMT_NLB_LB_ADDRESS_TYPE envsubst < <(cat $DIR/core/gloo-platform/self-signed/gloo-platform-mgmt-plane-2.4.yaml) | helm upgrade --install gloo-platform gloo-platform/gloo-platform \
            --kube-context=${MGMT_CONTEXT} \
            --namespace=gloo-mesh \
            --create-namespace \
            --version=${GLOO_PLATFORM_HELM_VERSION} \
...
EOF
    ...
    if [[ "$should_support_vault" == true ]]; then
        ...
        helm upgrade --install gloo-platform-crds gloo-platform/gloo-platform-crds \
            ...
            --create-namespace \
            --version=${GLOO_PLATFORM_HELM_VERSION}
        envsubst < <(cat $DIR/core/gloo-platform/vault/gloo-platform-agent-2.4.yaml) | helm upgrade --install gloo-platform-agent gloo-platform/gloo-platform \
            --kube-context=${EAST_CONTEXT} \
            --namespace=gloo-mesh \
            --create-namespace \
            --version=${GLOO_PLATFORM_HELM_VERSION} \
        ...
        helm upgrade --install gloo-platform-crds gloo-platform/gloo-platform-crds \
            ...
            --create-namespace \
            --version=${GLOO_PLATFORM_HELM_VERSION}
        envsubst < <(cat $DIR/core/gloo-platform/self-signed/gloo-platform-agent-2.4.yaml) | helm upgrade --install gloo-platform-agent gloo-platform/gloo-platform \
            --kube-context=${EAST_CONTEXT} \
            --namespace=gloo-mesh \
            --create-namespace \
            --version=${GLOO_PLATFORM_HELM_VERSION} \
...
if [[ "$should_deploy_integrations" == true ]]; then
    $DIR/integrations/provision-integrations.sh -p $EAST_CLOUD_PROVIDER -c $EAST_CONTEXT -n $EAST_CLUSTER -s alb
    $DIR/integrations/provision-integrations.sh -p $WEST_CLOUD_PROVIDER -c $WEST_CONTEXT -n $WEST_CLUSTER -s alb
    $DIR/integrations/provision-integrations.sh -p $MGMT_CLOUD_PROVIDER -c $MGMT_CONTEXT -n $MGMT_CLUSTER -s alb
    if [[ "$should_support_pca" == true ]]; then
